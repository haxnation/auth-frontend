...
export const Modal = {
    _createContainer(contentHtml) {
        ...
        el.innerHTML = `
            ...
            <div role="dialog" aria-modal="true" tabindex="-1" class="bg-white border-4 border-ink shadow-[12px_12px_0_0_#0b0b0b] p-0 max-w-md w-full rounded-none relative z-10 focus:outline-none">
                ${contentHtml}
            </div>
        `;
    ...
    alert(title, message, type = 'success') {
        return new Promise((resolve) => {
            ...
            const html = `
                <div class="${headerColor} p-4 font-mono flex justify-between uppercase font-bold tracking-widest border-b-4 border-ink">
    ...
    confirm(title, message, confirmText = 'Confirm', isDestructive = false) {
        return new Promise((resolve) => {
            ...
            const html = `
                <div class="bg-ink text-white p-4 font-mono flex justify-between uppercase font-bold tracking-widest border-b-4 border-ink">
    ...
    showQR(user) {
        ...
        const html = `
            <div class="bg-ink text-cyan p-4 font-mono flex justify-between uppercase font-bold tracking-widest border-b-4 border-ink">
            ...
            <div class="p-8 text-center bg-canvas">
                <div class="w-16 h-16 bg-ink border-2 border-ink text-cyan shadow-[4px_4px_0_0_#0b0b0b] flex items-center justify-center mx-auto mb-6 text-3xl font-bold rounded-none">
                    ${user.name.charAt(0).toUpperCase()}
                </div>
                
                <h3 class="text-2xl font-bold text-ink uppercase tracking-tight mb-2">${user.name}</h3>
                <p class="font-mono text-[10px] text-gray-500 mb-6 uppercase tracking-widest border-2 border-ink bg-ink text-white px-2 py-1 inline-block shadow-[2px_2px_0_0_#0b0b0b]">AWAITING_SCAN</p>
